...
export class TrackingApiService {
  ...
  /** POST /tracking/location — envoyer la position GPS */
  sendLocation(data: CreateLocationTrack): Observable<ApiResponse<LocationTrack>> {
    return this.http.post<ApiResponse<LocationTrack>>(`${this.api}/location`, data);
  }

  /** GET /tracking/live — positions en temps réel de tous les commerciaux */
  getLivePositions(): Observable<ApiResponse<UserLivePosition[]>> {
    return this.http.get<ApiResponse<UserLivePosition[]>>(`${this.api}/live`);
  }

  /** POST /tracking/check-in — check-in visite avec coordonnées GPS */
  ...
  /** POST /tracking/check-out — check-out visite avec coordonnées GPS */
  ...
  /** GET /tracking/history/:userId — historique des positions d'un utilisateur */
